...
import { ref } from 'vue';
import { planningSupportQuickTags } from '@/mock/planning';
import type { PlanningAnalysisStep, PlanningChatMessage } from './types';

defineOptions({
  name: 'PlanningSupportAiPanel'
});
...
const messages = ref<PlanningChatMessage[]>([
  {
    id: 'support-welcome',
    role: 'assistant',
    content: '您好，我是机动保障方案AI助手。请设置左侧保障参数，我将为您智能生成保障方案与资源调配建议。'
  }
]);
...
function handleSend() {
  ...
  if (!text) return;
  emit('send', text);
  messages.value.push({ id: `support-u-${Date.now()}`, role: 'user', content: text });
  chatInput.value = '';
}

function handleQuickAction(label: string) {
  const msg = `请${label}`;
  messages.value.push({ id: `support-q-${Date.now()}`, role: 'user', content: msg });
  emit('send', msg);
}
...
<template>
  <div class="support-ai-panel" :class="{ 'support-ai-panel--collapsed': collapsed }">
    <!-- 标题栏 -->
    <div class="ai-header">
      <span class="ai-icon">🤖</span>
      <div class="ai-title-group">
        <span class="ai-title">AI智能助手</span>
        <span class="ai-subtitle">机动保障方案助手</span>
      </div>
    ...
    <div v-show="!collapsed" class="ai-body">
      <!-- 欢迎语 -->
      <div class="greeting-box greeting-box--green">
      ...
      <div class="action-tags">
        <button
          v-for="tag in planningSupportQuickTags"
          :key="tag.label"
          type="button"
          class="action-tag-btn action-tag-btn--green"
      ...
      <div class="analysis-section analysis-section--green">
        ...
        <div class="progress-bar-wrap">
          <div class="progress-bar">
            <div class="progress-bar__fill progress-bar__fill--green" :style="{ width: `${progress}%` }" />
          </div>
          <span class="progress-percent progress-percent--green">{{ progress }}%</span>
        </div>
...
.support-ai-panel--collapsed .ai-body,
.support-ai-panel--collapsed .chat-input-area {
  display: none;
}
