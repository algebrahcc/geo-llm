...
import type { PlanningLayerItem, PlanningLayerKey } from './types';

defineOptions({
  name: 'PlanningLayerPanel'
});

defineProps<{
  collapsed: boolean;
  layers: PlanningLayerItem[];
}>();

const emit = defineEmits<{
  change: [payload: { key: PlanningLayerKey; visible: boolean }];
  toggleCollapse: [];
}>();
</script>
...
<template>
  <NCard
    ...
    :content-style="{ maxHeight: 'calc(100vh - 300px)', overflowY: 'auto' }"
  >
    <template #header>
...
.layer-panel :deep(.n-card__content::-webkit-scrollbar-thumb:hover) {
