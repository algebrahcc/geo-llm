...
/** 行内启用/禁用：开关即时生效，无需进入弹窗 */
function renderStatusSwitch(row: Api.System.UserItem) {
  const on = row.status === 1;
  return h('div', { class: 'sys-status-switch' }, [
    h(NSwitch, {
      value: row.status,
      size: 'small',
      'aria-label': '启用状态',
      'onUpdate:value': (value: string | number) => handleToggleStatus(row, Number(value))
    }),
    h('span', { class: `sys-status-text ${on ? 'is-on' : 'is-off'}` }, on ? '启用' : '禁用')
  ]);
}

/** 先取详情再提交，避免只提交部分字段导致角色/部门被清空 */
async function handleToggleStatus(row: Api.System.UserItem, value: number) {
  const { data } = await fetchUserDetail(row.id);
  if (!data) return;
  const { error } = await fetchUserUpdate(row.id, {
...
const columns: DataTableColumns<Api.System.UserItem> = [
  ...
  { title: '手机号', key: 'phone', width: 130, render: row => h('span', { class: 'sys-mono' }, row.phone || '—') },
  { title: '部门', key: 'deptName', width: 120 },
  { title: '状态', key: 'status', width: 120, align: 'center', render: renderStatusSwitch },
  {
    title: '创建时间',
    key: 'createTime',
    width: 180,
...
async function handleBatchDelete() {
  ...
  const { error } = await fetchUserDelete(ids);
  if (!error) {
  ...
  <div class="sys-page">
    ...
    <section class="sys-search-card">
      ...
      <div class="sys-search-actions">
        <NPopconfirm v-if="checkedRowKeys.length" @positive-click="handleBatchDelete">
          <template #trigger>
            <NButton type="error">
              <template #icon>
          ...
          确认删除选中的 {{ checkedRowKeys.length }} 个用户？
        </NPopconfirm>
    ...
    <section class="sys-content-card">
      ...
      >
        <template #empty>
          <EmptyState icon="mdi:account-search-outline" title="暂无用户" description="没有找到符合条件的用户" />
        </template>
      </NDataTable>
    </section>
