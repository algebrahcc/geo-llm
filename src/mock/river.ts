...
export const riverPlanSummaries = {
  'plan-a': {
    ...
    summary:
      '西岸狮子头滩地集结，门桥编组于关渡大桥上游约1km处漕渡横越实测491m河幅，在关渡宫南侧登陆场上岸，优先保证主突击群过河时效。',
    metrics: [
...
export const riverPlanScenes = {
  'plan-a': {
    route: {
      ...
      name: '方案一渡河路线',
      color: '#63e6be',
      // 八里岸集结 → 狮子头下水 → 横渡（水线坐标为 OSM 水域面量算值） → 关渡宫南侧登陆 → 纵深机动
      positions: [
        [121.4468, 25.1192],
        [121.4515, 25.1196],
        [121.45705, 25.1202],
    ...
    // 风险区色块标绘已按演示反馈移除（半透明区域难以理解），风险描述保留在方案卡片与点位表
    riskZones: [],
    marks: [
      {
        id: 'plan-a-mark-1',
  ...
  'plan-b': {
    route: {
      ...
      name: '方案二渡河路线',
      color: '#5ea4ff',
      // 五股器材转换场 → 西岸桥头 → 浮桥桥位（河心） → 社子岛头接引 → 岛内纵深（水线为量算值）
      positions: [
        [121.448, 25.113],
        [121.45901, 25.11328],
        [121.46142, 25.11364],
  ...
  'plan-c': {
    route: {
      ...
      name: '方案三渡河路线',
      color: '#f7b267',
      // 八里渡船头下水（水线） → 横渡 → 竹围岸段多点登陆 → 向淡水方向纵深（水线为量算值）
      positions: [
        [121.436, 25.142],
        [121.44692, 25.14525],
        [121.45166, 25.1467],
...
export const crossingPlanCards: CrossingPlanCard[] = [
  {
    ...
    routeDesc: '八里狮子头滩地集结，于关渡大桥上游约1km渡场漕渡横越实测491m，在关渡宫南侧登陆场上岸',
